...
from HumanoidNavigation.Utils.footstep_planner import FootstepPlanner, Foot
...
class DifferentialDriveMPC(MpcSkeleton):
    def __init__(self, state_dim, control_dim, N_horizon, N_simul, sampling_time, goal=None, cbf=None):
        ...
        if self.goal is None:
            self.reference = self.optim_prob.parameter(self.state_dim - 1,
                                                       self.N_horizon)  # in this case we are only defining a cartesian trajectory
    ...
    def add_constraints(self):
        ...
        for k in range(self.N_horizon):
            self.optim_prob.subject_to(
                self.X_mpc[:, k + 1] == self.X_mpc[:, k] + self.sampling_time * self.integrate(self.X_mpc[:, k],
                                                                                               self.U_mpc[:, k]))
    ...
    def cost_function(self):
        ...
        if self.goal is None:  # hence we have trajectory tracking
            ...
            terminal_cost = cs.sumsqr(
                self.X_mpc[:2, -1] - self.reference[:, -1])  # not sure if it's -1 or -2 (double check this)
            weight = 200
    ...
    def simulation(self, ref=None):
        ...
        for k in range(self.N_simul):
            ...
            if self.goal is None and ref is not None:
                self.optim_prob.set_value(self.reference, ref[:,
                                                          k:k + self.N_horizon])  # set the actual reference equal to the reference from k to k+N_horizon
            ...
            X_pred[:, k + 1] = X_pred[:, k] + self.sampling_time * self.integrate(X_pred[:, k],
                                                                                  U_pred[:, k]).full().squeeze(-1)
...
if __name__ == "__main__":
    ...
    if problem != "tracking":
        ...
        # Compute the footstep plan (pred_traj needs to be sampled, because the associated sampling time is too small)
        footsteps_plan = FootstepPlanner().compute_plan_from_uni_state(unicycle_state=pred_traj.T[::50],
                                                                       initial_support_foot=Foot.RIGHT)
        # Show the footstep plan
        fig = plt.gcf()  # Get current figure
        ax = plt.gca()  # Get current axis
        FootstepPlanner.plot_steps(footsteps_plan, fig, ax)
