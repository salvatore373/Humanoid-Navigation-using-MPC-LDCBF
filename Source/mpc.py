...
class Mpc():
    ...
    def __init__(self, n, m, N, N_simul, sampling_time, goal=None, reference=None):
        ...
        # predicted trajectory over the simulation
        ...
        if self.reference is not None:
            ...
            for k in range(self.N):
                reference_cost += self.x[1:, k] - self.reference[:, k] # xy trajectory
            terminal_cost = self.x[1:, N] - self.reference[:, N]
            cost_function += cs.sumsqr(reference_cost) + cs.sumsqr(terminal_cost)
    ...
    def simulation(self):
        ...
        print('The average computation time is: ', np.mean(iter_time) * 1000, ' ms')
...
delta_t = 0.01
mpc = Mpc(n=3, m=2, N=40, N_simul=300, sampling_time=delta_t, goal=(-2,-3,-cs.pi/2))
mpc.add_constraints()
mpc.simulation()
mpc.plot()

# psi = np.linspace(0, 2*np.pi, int(1/delta_t))
# reference = np.array([[1+np.cos(psi)],[2+np.sin(psi)]]).squeeze(1)
# mpc = Mpc(n=3, m=2, N=40, N_simul=300, sampling_time=delta_t, reference=reference)
# mpc.add_constraints()
# mpc.simulation()
# mpc.plot()
