...
It does so by projecting the displacement of the CoM position on the longitudinal and lateral axes, and ensuring that its value lies between tuned upper and lower bounds, indicating the maximum distance reachable by the swing foot in both directions.
From the constraint described in the reference paper~\cite{peng_main_paper}, we did some modifications; in
particular we adopted a better tailored version from Cipriano et al.~\cite{CIPRIANO2023104495}.
The differences between the implemented constraint and the one proposed by Peng et al. relies in:
\begin{itemize}
    \item asymmetric upper and lower bounds: $l_{x,\min},l_{y,\min},l_{x,\max},l_{y,\max}$
    \item a vector controlling the minimum distance from the previous foot: $(0,\;\; s_v*l)$
\end{itemize}
where, as mentioned in previous sections, $s_v$ defines which foot is the stance; while the term $l$ controls the
minimum distance from the previous step; in our work we have $l=0.05$.
