...
assume that there exists a \todo{Add footnote} class $\mathcal{K}$ function $\gamma$ such that
$$
...
It means that, if the state starts from the safety set $S$, there should exists an input that, when applied, produces a state which will still be in $S$: \todo{Not sure about it} namely, $S$ is invariant.
