...
Although the evolution of the variables $\theta$ and
$\omega$ is clearly visible, it is worth providing a brief commentary, especially given the densely packed environment.
Their trajectories exhibit distinct peaks, which can be directly linked to the robot's path; in fact, the first and
second main peaks correspond to the robot encountering obstacles. As the robot detects obstacles at different points
along its path, it must execute rapid lateral movements to avoid collisions while simultaneously minimizing the
error relative to its goal.
